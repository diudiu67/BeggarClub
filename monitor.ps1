# BeggarClub Health Monitor - runs every 5 minutes via watchdog-loop.ps1
...
# ⚠️  DO NOT DELETE — part of the auto-recovery chain (see .claude/CLAUDE.md)
...
$LOG_MAX_LINES  = 2000   # keep last ~1 week of entries
...
function Update-GitHubPages($tunnelUrl) {
    try {
        ...
        $html = "<!DOCTYPE html><html><head><meta charset=`"utf-8`"><title>BeggarClub Music</title>" +
                "<script>window.location.replace(`"$tunnelUrl`" + window.location.search);</script></head>" +
...
# ── 5b. Bot thread liveness check ─────────────────────────────────────────────
# Uvicorn is up, but the bot daemon thread inside it could have crashed silently.
# /api/health now exposes bot_alive=false when the thread has exited.
try {
    $healthJson = Invoke-RestMethod -Uri "http://localhost:8080/api/health" -TimeoutSec 10 -ErrorAction Stop
    if ($healthJson.PSObject.Properties.Name -contains "bot_alive" -and $healthJson.bot_alive -eq $false) {
        Write-Log "Bot thread dead (bot_alive=false) — restarting uvicorn to recover"
        $lastErr = ""
        if (Test-Path "$backendDir\bot_err.log") {
            $lastErr = (Get-Content "$backendDir\bot_err.log" -ErrorAction SilentlyContinue | Select-Object -Last 5) -join " | "
        }
        if (Test-CanRestart $tracker "uvicorn") {
            Stop-Port8080
            $restarted  = Start-Uvicorn
            $n = $tracker["uvicorn_count"]
            if ($restarted) {
                Write-Log "Uvicorn restart #$n OK (bot thread recovery)"
                $dmMsg = "[BOT THREAD DEAD] Bot's internal thread crashed silently. Uvicorn restarted (#$n) — service restored."
                if ($lastErr) { $dmMsg += "`nLast error: $lastErr" }
                Send-DiscordDM $dmMsg
            } else {
                Write-Log "Uvicorn restart #$n FAILED (bot thread recovery)"
                $dmMsg = "[BOT THREAD DEAD] Bot thread crashed. Uvicorn restart #$n failed — manual intervention required."
                if ($lastErr) { $dmMsg += "`nLast error: $lastErr" }
                Send-DiscordDM $dmMsg
            }
        } else {
            Write-Log "Bot thread dead — crash loop limit reached, auto-restart suspended"
            Send-DiscordDM "[BOT CRASH LOOP] Bot thread has crashed repeatedly. Auto-restart suspended — please check manually."
        }
        exit
    }
} catch {
    Write-Log "Bot liveness parse failed: $_"
}
